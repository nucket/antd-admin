...
class Thermometer extends Component {
  ...
  render() {
    ...
    // Revisar esta constante, en la formula esta el error del valor en el termometro
    // alternativa : { height: this.options.percent() + '%' };
  ...
  _generateOptions() {
    return {
      ...
      reverseGradient: this.props.reverseGradient || true, // default true,
  ...
  _createIntervals() {
    if (this.options.steps) {
      for (let step = 0; step <= this.options.steps; step++) {
        let value = ((this.options.max / this.options.steps) * step).toFixed(0);
